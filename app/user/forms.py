...
from wtforms.validators import DataRequired, Regexp, Email, EqualTo, Length, NumberRange, ValidationError
...
def email_or_username():
    ...
    message = f'Please enter a valid username or email'
...
def valid_username():
    ...
    message = f'Username should consist of A-z, a-z, 0-9 and _'
...
class SignupForm(FlaskForm):
    ...
    dob = DateField(
        "Date of Birth", validators=[DataRequired(), dob_range()]
    )
    # TODO: username should consist of Uppercase, LowerCase, Digits, _
    username = StringField(
        "Username", validators=[DataRequired(), Length(min=3, max=25), valid_username()]
    )
    email = StringField(
    ...
    country_code = StringField(
        "Country Code", validators=[DataRequired(), Length(min=1, max=6)]
    )
    # TODO: validation
    mobile_number = IntegerField(
        "Mobile Number", validators=[DataRequired()]
    )
    # TODO: add rules for password (Upper case, Lower Case, Digit, Special Character)
    password = PasswordField(
        "Password", validators=[DataRequired(), Length(min=8, max=40)] 
    )
    confirm = PasswordField(
        "Verify Password", validators=[DataRequired(), EqualTo("password", message="Password desn't match with confirm password")]
    )
    billing_address = StringField(
        "Billing Address", validators=[DataRequired(), Length(min=3, max=80)]
    )
    shipping_address = StringField(
        "Shipping Address", validators=[DataRequired(), Length(min=3, max=80)]
    )
    # TODO: better validator
    zip_code = IntegerField(
        "Zip Code", validators=[DataRequired(), NumberRange(min=9999, message="Zip Code must be 5 digit")]
    )
...
class LoginForm(FlaskForm):
    ...
    username = StringField("Username/Email", validators=[DataRequired(), email_or_username()])
